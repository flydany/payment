    ...
    <div class="search flyer-form pane" id="info-search">
        <div class="form-item">
            ...
            <div class="item-inline">
                ...
                <div class="input-inline w-100px"><?= Render::select('platform', Recharge::$platformSelector, null, ['prompt' => '全部--', 'flyer' => 'select', 'class' => 'tabler']) ?></div>
            </div>
            <div class="item-inline">
                <div class="input-title">充值状态</div>
                <div class="input-inline w-100px"><?= Render::select('status', Recharge::$statusSelector, null, ['prompt' => '全部--', 'flyer' => 'select', 'class' => 'tabler']) ?></div>
            </div>
            <div class="item-inline">
        ...
        <table id="info-table" cellspacing="0" cellpadding="0">
            <thead>
                <tr>
                    ...
                    <th><i class="icon-time"></i> 充值时间</th>
...
<script>
    $(document).ready(function() {
        ...
        (new tabler).init({
            // 请求地址
            url: '<?= Url::to('@web/recharge/recharge-list') ?>',
            // 数据渲染配置
    ...
    {{if infos != undefined && infos.length}}
        {{each infos as info key}}
            <tr id="tr-{{info.id}}" data-id="{{info.id}}">
                ...
                <td>
                    <a class="flyer-status blue thin" href="<?= Url::to('@web/recharge/recharge-detail?id=') ?>{{info.id}}"><i class="icon-eye-open icon-large" title="详情/审核"></i>详情/审核</a>
                </td>
            </tr>
        {{/each}}
    {{else}}
